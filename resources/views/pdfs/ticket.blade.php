...
<div class="ticket-wrapper">
    <div>&nbsp;</div>
    <div class="ticket-background" @if($design->getMedia()->count() > 0) style="background: url({{
        str_replace(env('APP_URL'),'',$design->image()) }}) no-repeat center; background-size: contain;" @endif></div>
    <div class="content">
        <div class="separator" style="top: 80px;"></div>
        <div class="dashed" style="top: 345px;"></div>
        @if($design->ticketn_exists)
        <h2 class="position-absolute"
            style="@if($design->ticketn_r) right: {{ $design->ticketn_r }}px; @else left: {{ $design->ticketn_l }}px; @endif top: {{ $design->ticketn_t }}px;">
            Электронный билет | E-Ticket # {{ $ticket->id }}</h2>
        @endif
        ...
        <h1 class="position-absolute"
            style="@if($design->title_r) right: {{ $design->title_r }}px; @else left: {{ $design->title_l }}px; @endif top: {{ $design->title_t }}px;">
            {{ $event->title }}</h1>
        @endif
        ...
        <div class="position-absolute right-img-content"
            style="@if($design->user_r) right: {{ $design->user_r }}px; @else left: {{ $design->user_l }}px; @endif top: {{ $design->user_t }}px;">
            <b>Имя: </b>{{ $ticket->order->name }}
        </div>
        @endif
        ...
        <div class="position-absolute right-img-content"
            style="@if($design->venue_r) right: {{ $design->venue_r }}px; @else left: {{ $design->venue_l }}px; @endif top: {{ $design->venue_t }}px;">
            <b>Место: </b>{{ \Illuminate\Support\Str::limit($venue->title, 30, '...') }}
        </div>
        @endif
        ...
        <div class="position-absolute"
            style="@if($design->barcode_r) right: {{ $design->barcode_r }}px; @else left: {{ $design->barcode_l }}px; @endif top: {{ $design->barcode_t }}px;">
        ...
        @if($design->qr_exists)
        <div class="position-absolute"
            style="@if($design->qr_r) right: {{ $design->qr_r }}px; @else left: {{ $design->qr_l }}px; @endif top: {{ $design->qr_t }}px;">
            {!! DNS2D::getBarcodeHTML($ticket->barcode,"QRCODE", 4, 4, "black") !!}
        </div>
        ...
        <div class="rules position-absolute" style="left: 119px; top: 350px; width: 900px; font-size: 8px;">
            <div class="box">
                <div class="w-50">
        ...
        <div class="position-absolute" style="left: 119px; top: 1110px; line-height: 1.4">
            ...
            <div>Площадка: <b>{{ \Illuminate\Support\Str::limit($venue->title, 65, '...') }} ({{
                    $timetable->scheme->title }})</b></div>
...
<style>
    ...
    .box div{width:300px;display:inline-block;}
    .w-50 p {
